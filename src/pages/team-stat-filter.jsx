    ...
    const teamData = teams.map(team => {
      if (this.props.state.page === 'general') {
        ...
        <tr key={team.team}>
          ...
          <td>{getPointDifferential(team.team).pointsFor - getPointDifferential(team.team).pointsAgainst}</td>
        </tr>
        );
      }
          ...
          <tr key={team.team}>
            ...
            <td>
              {(getOpposingStats(team.team).twoMakes + getOpposingStats(team.team).threeMakes) + '/' +
                (getOpposingStats(team.team).twoAttempts + getOpposingStats(team.team).threeAttempts)}
            </td>
            <td>
              {((getOpposingStats(team.team).twoMakes + getOpposingStats(team.team).threeMakes) /
                (getOpposingStats(team.team).twoAttempts + getOpposingStats(team.team).threeAttempts) * 100).toFixed(2) + '%'}
            </td>
            <td>
              {(getOpposingStats(team.team).threeMakes) + '/' + getOpposingStats(team.team).threeAttempts}
            </td>
            <td>
              {((getOpposingStats(team.team).threeMakes / getOpposingStats(team.team).threeAttempts) * 100).toFixed(2) + '%'}
            </td>
            <td>
              {(getOpposingStats(team.team).ftMakes) + '/' + getOpposingStats(team.team).ftAttempts}
            </td>
            <td>
              {((getOpposingStats(team.team).ftMakes / getOpposingStats(team.team).ftAttempts) * 100).toFixed(2) + '%'}
            </td>
          </tr>
        );
      }
